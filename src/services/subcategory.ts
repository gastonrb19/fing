...
export class SubcategoryService {
    ...
    async findByCategory(categoryId: number, pagination: Pagination): Promise<SubcategoryEntity[]> {
        //Validate that the parent category exists;
        const category = await myDataSource.getRepository(CategoryEntity).findOneBy({ id: categoryId });
        if(!category){
    ...
    async findOneById(id: number): Promise<SubcategoryEntity> {
        const getSubcategory = await myDataSource.getRepository(SubcategoryEntity).findOne({
            where: { id },
            relations: { category: true },
        });
        if(!getSubcategory){
    ...
    async create(createSubcategoryDTO: createSubcategoryDTO): Promise<SubcategoryEntity> {
        ...
        const subcategoryName = await myDataSource.getRepository(SubcategoryEntity).findOneBy({ name: createSubcategoryDTO.name });
        if(subcategoryName){
            throw new GeneralError(`Subcategory with name ${createSubcategoryDTO.name} already exist`, 400);
        }

        //Check that the parent category exists;
        const category = await myDataSource.getRepository(CategoryEntity).findOneBy({ id: createSubcategoryDTO.categoryId });
        if(!category){
        ...
        return await myDataSource.getRepository(SubcategoryEntity).save(newSubcategory);
    }
    ...
    async update(id: number, updateSubcategoryDTO: updateSubcategoryDTO): Promise<SubcategoryEntity> {
        ...
        if(updateSubcategoryDTO.name && updateSubcategoryDTO.name !== subcategory.name){
            const subcategoryName = await myDataSource.getRepository(SubcategoryEntity).findOneBy({ name: updateSubcategoryDTO.name });
            if(subcategoryName){
                throw new GeneralError(`Subcategory with name ${updateSubcategoryDTO.name} already exist`, 400);
            }
        }

        //If the category changes, validate that the new one exists;
        if(updateSubcategoryDTO.categoryId){
            const category = await myDataSource.getRepository(CategoryEntity).findOneBy({ id: updateSubcategoryDTO.categoryId });
            if(!category){
        ...
        return await myDataSource.getRepository(SubcategoryEntity).save(subcategory);
    }
